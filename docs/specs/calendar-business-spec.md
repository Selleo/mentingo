...
Calendar gives learners, trainers, and administrators one place to see time-based learning commitments. It combines scheduled live training, mandatory course due dates, and a user's Microsoft Outlook events so users do not need to inspect separate systems to understand what is coming up.
...
- Microsoft 365 users who want a read-only view of their own Outlook calendar alongside Mentingo commitments.
...
The Outlook connection is personal to each user. Imported events are not editable in Mentingo and are not made visible to tenant administrators or other users.
...
A user can connect one Microsoft account from the Integrations tab in settings. Mentingo requests read-only calendar access, performs an initial import, and then keeps a rolling window from 30 days in the past through six months in the future synchronized through Microsoft change notifications, incremental synchronization, and scheduled reconciliation. Selecting a different Microsoft account requires explicit replacement confirmation.
