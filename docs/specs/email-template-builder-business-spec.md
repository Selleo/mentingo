...
The editor offers only the variables supplied by that event, with sample values for test email. Dynamic variables have visible descriptions in both the sidebar and insertion dropdown, explaining when text disappears or changes with the deadline, course context, or certificate state. Deadline reminders and certificate notices include variables for localized wording computed when the email is sent. They reflect the actual days remaining, certificate availability, and whether a certificate expired or was manually reset. Unchanged wording in previously copied defaults is resolved the same way during rendering; custom wording is preserved. The administrator can save incomplete work as a draft. Publishing requires a complete base-language subject and body plus valid block settings, links, images, and variables. Complete additional translations become active at the same time; incomplete translations use the base language for recipients.
...
- API responses expose supported variables and sample values. Authentication emails require a usable action link in every complete translation, and errors/warnings have translations in all supported UI languages. Account-access tokens may appear only as the complete destination of a button or text link. Subjects, visible text, image fields, and composed external URLs cannot contain those tokens. Saves, previews, test sends, and published delivery enforce the restriction. Previously saved unsafe templates fail validation at delivery and must be corrected before they can send.
...
### Security and dynamic wording regressions

Focused unit regressions cover credential exposure across all four account-access events, including external images and links, subjects, labels, footer content, and tokens split across rich-text formatting. They also verify valid standalone links and rejection of unsafe persisted templates before asset resolution.

Rendering regressions compare assignment, inactivity, deadline, and certificate wording with the existing code-defined emails in all seven languages. They cover assignments with and without dates, course-specific and platform-level inactivity (including the short-inactivity subject and action), same-day, next-day, and longer deadlines, both certificate-availability branches, and expiry versus manual reset. Additional cases cover whole-email language fallback and unchanged legacy default wording while preserving administrator edits. These are service and rendering tests, not live SMTP or browser E2E evidence.
