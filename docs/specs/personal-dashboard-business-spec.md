...
The retired unfinished-courses card is not available. Restore default uses explicit role profiles for both order and size. Student profiles start with Continue Learning `2x2`, AI Mentor `2x2`, and Calendar `4x2`, followed by To-do, Required Courses, Course Completion, and Certificates at `2x2`. Administrator and Content Creator profiles start with Deadline Risks `2x2`, Calendar `4x2`, and Training Completion `2x2`, followed by AI Mentor `3x2` and To-do `3x2`. Group Manager profiles start with Calendar, Deadline Risks, and Training Completion. Restored profiles never use a one-row card; compact one-row sizes remain a manual personalization option where supported. Defaults are filtered by actual permissions, and temporarily inaccessible widgets remain in saved preferences so they can return if the user regains access.
