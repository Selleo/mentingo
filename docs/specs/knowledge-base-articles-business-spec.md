...
When an article is published, Mentingo requires a title for every language currently attached to that article. Learners browse published articles through the section table of contents. Public visitors can open only articles allowed by both tenant configuration and the article's public setting.
