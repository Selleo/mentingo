...
For HR and L&D customers, the value is safer support. Platform staff can inspect tenant-facing workflows as the specific administrator, learner, trainer, content creator, or custom-role user affected by an issue while Mentingo clearly marks the session as temporary support access, hides normal super-admin navigation inside the target tenant, suppresses personal account settings, and provides a visible exit path.
...
- Tenant admins, learners, trainers, and content creators benefit from faster troubleshooting while their own accounts remain private.
...
A managing admin opens the tenant list and clicks **Impersonate user** for a target tenant. The selector opens on an administrator list with name/email search. If the search does not find an administrator, the operator can switch to **All users** without retyping the search and optionally filter by a target-tenant role. Mentingo creates a short-lived grant and redirects the browser to the target tenant's support callback. The callback exchanges the grant for support-mode cookies that carry support context rather than a normal tenant-user login.

Inside the target tenant, Mentingo shows a warning banner with a countdown and exit button. The impersonated user receives that user's effective roles and permissions, so the operator sees the same permitted experience as the selected person. The app hides super-admin navigation, suppresses the profile link and account settings, and redirects support-mode users away from super-admin routes.
...
- Support session creation and target-user discovery are exposed from `apps/api/src/super-admin/tenants.controller.ts` and implemented in `apps/api/src/support-mode/support-mode.service.ts`.
- Managing-admin authorization remains enforced by `tenant.manage` and the managing-tenant guard; the selected user's roles never grant the source operator additional access.
...
Frontend E2E coverage verifies that a managing admin can enter support mode, lands in the target tenant, sees the support banner and timer, cannot use super-admin navigation there, exits back to tenant administration, and does not see account settings while in support mode. The implementation adds coverage for selecting a non-admin target and preserving that target's permissions. Source evidence covers pending grants, activation, one-hour expiry, revocation, expired-session cleanup, and support-mode enter events. Focused backend tests cover target-user eligibility and selector query behavior; direct API E2E coverage for tenant isolation and forged targets remains a validation risk if the shared E2E environment is unavailable.
