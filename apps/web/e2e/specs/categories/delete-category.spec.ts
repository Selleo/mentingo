...
test("admin can delete a single category from the categories list", async ({
  factories,
  withWorkerPage,
}) => {
  await withWorkerPage(USER_ROLE.admin, async ({ page }) => {
    const categoryFactory = factories.createCategoryFactory();
    const category = await categoryFactory.create(`Delete Category ${Date.now()}`);

    await openCategoriesPageFlow(page);
    await filterCategoriesFlow(page, { title: category.title });
    await selectCategoriesFlow(page, [category.id]);
    await confirmDeleteCategoriesFlow(page);

    await expect
      .poll(async () => {
        const deletedCategory = await categoryFactory.findByTitle(category.title);
        return deletedCategory;
      })
      .toBeNull();
    await expect(page.getByTestId(CATEGORIES_PAGE_HANDLES.DELETE_DIALOG)).toHaveCount(0);
    await expect(page.getByTestId(CATEGORIES_PAGE_HANDLES.row(category.id))).toHaveCount(0);
