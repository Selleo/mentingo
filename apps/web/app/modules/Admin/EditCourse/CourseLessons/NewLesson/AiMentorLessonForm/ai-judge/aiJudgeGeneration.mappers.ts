...
const mapValidationToChecks = (
  ...
  return validation.issues.map((issue, index) => {
    ...
    return {
      id: `${issue.code}-${index}`,
      label: replaceInternalReferences(issue.message, configuration, t),
      detail: replaceInternalReferences(issue.correction, configuration, t),
      ...targetContext,
...
const mapAttemptHistory = (
  ...
  progress.attemptHistory.map(({ attempt, changes, validation }) => ({
    ...
    summary: replaceInternalReferences(validation.summary, configuration, t),
    corrections: validation.issues.map(({ correction }) =>
      replaceInternalReferences(correction, configuration, t),
    ),
...
export const mapAiJudgeGenerationSnapshotToViewState = (
  ...
  const humanizedFirstCorrection = firstConcern
    ? replaceInternalReferences(firstConcern.correction, configuration, t)
    : undefined;
  const validationConcern = firstConcern?.correction ?? validation?.summary;
