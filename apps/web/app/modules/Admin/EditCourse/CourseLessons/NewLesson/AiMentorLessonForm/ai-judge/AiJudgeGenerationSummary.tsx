...
const formatTaskGoalSummary = (value: string) =>
...
const hasVisibleChange = (change: AiJudgeGenerationChange) =>
  formatChangeValue(change.before) !== formatChangeValue(change.after);

const getChangeFieldLabelKey = (change: AiJudgeGenerationChange) => {
  switch (change.field) {
...
const shouldShowChangeFieldLabel = (change: AiJudgeGenerationChange) => {
...
const groupChangesByTarget = (changes: AiJudgeGenerationChange[]) => {
...
const groupChecksByTarget = (checks: AiJudgeGenerationCheck[]) => {
...
const isInternalTargetReference = (targetRef: string | undefined) =>
  targetRef ? /^[CB]\d+$/.test(targetRef) : false;

const isCriterionTargetReference = (targetRef: string | undefined) =>
...
const AiJudgeGenerationTargetHeader = ({
  ...
  compact = false,
}: Pick<AiJudgeGenerationCheck, "targetLabel" | "targetRef" | "targetScore" | "targetTypeLabel"> & {
