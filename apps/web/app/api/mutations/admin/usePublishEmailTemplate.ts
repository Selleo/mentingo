...
export function usePublishEmailTemplate() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const response = await ApiClient.api.emailNotificationTemplatesControllerPublishTemplate(id);
    ...
    onSuccess: (_data, id) => {
      ...
      toast({
        variant: "default",
        description: t("emailTemplates.toast.publishedSuccessfully"),
      });
    },
    onError: (error) => {
      toast({
        ...
        description: getTranslatedApiErrorMessage(
          error,
          t,
          t("emailTemplates.toast.publishFailed"),
        ),
      });
    },
  });
