...
export function useUnarchiveEmailTemplate() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const response =
        await ApiClient.api.emailNotificationTemplatesControllerUnarchiveTemplate(id);
    ...
    onSuccess: (_data, id) => {
      ...
      toast({
        variant: "default",
        description: t("emailTemplates.toast.unarchivedSuccessfully"),
      });
    },
    onError: (error) => {
      toast({
        ...
        description: getTranslatedApiErrorMessage(
          error,
          t,
          t("emailTemplates.toast.unarchiveFailed"),
        ),
      });
    },
  });
