...
type DeleteArticleSectionOptions = {
  sectionId: string;
};

export function useDeleteArticleSection() {
  const { toast } = useToast();
  const { t } = useTranslation();

  return useMutation({
    mutationFn: async ({ sectionId }: DeleteArticleSectionOptions) => {
      await ApiClient.api.articlesControllerDeleteArticleSection(sectionId);
    },
    onSuccess: async () => {
