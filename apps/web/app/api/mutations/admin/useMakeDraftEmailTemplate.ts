...
export function useMakeDraftEmailTemplate() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const response =
        await ApiClient.api.emailNotificationTemplatesControllerMakeTemplateDraft(id);
    ...
    onSuccess: (_data, id) => {
      ...
      toast({
        variant: "default",
        description: t("emailTemplates.toast.madeDraftSuccessfully"),
      });
    },
    onError: (error) => {
      toast({
        ...
        description: getTranslatedApiErrorMessage(
          error,
          t,
          t("emailTemplates.toast.makeDraftFailed"),
        ),
      });
    },
  });
