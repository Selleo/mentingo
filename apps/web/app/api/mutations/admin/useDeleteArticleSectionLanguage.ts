...
type DeleteArticleSectionLanguageOptions = {
  id: string;
  language: SupportedLanguages;
};

export function useDeleteArticleSectionLanguage() {
  const { toast } = useToast();
  const { t } = useTranslation();

  return useMutation({
    mutationFn: async ({ id, language }: DeleteArticleSectionLanguageOptions) => {
      await ApiClient.api.articlesControllerDeleteArticleSectionLanguage(id, { language });
    },
    onSuccess: async () => {
