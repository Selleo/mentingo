...
type AddArticleSectionLanguageOptions = {
  id: string;
  language: SupportedLanguages;
};

export function useAddArticleSectionLanguage() {
  const { toast } = useToast();
  const { t } = useTranslation();

  return useMutation({
    mutationFn: async ({ id, language }: AddArticleSectionLanguageOptions) => {
      const response = await ApiClient.api.articlesControllerAddNewLanguageToSection(id, {
        language,
      });
