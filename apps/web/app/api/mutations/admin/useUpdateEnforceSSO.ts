...
export function useUpdateEnforceSSO() {
  const { toast } = useToast();
  const { t } = useTranslation();

  return useMutation({
    mutationFn: async () => {
      const { data } = await ApiClient.api.settingsControllerUpdateEnforceSso();
    ...
    onSuccess: () => {
      ...
      toast({
        variant: "default",
        description: t("ssoEnforcementView.toast.ssoEnforcementUpdatedSuccessfully"),
      });
    },
    onError: (error) => {
