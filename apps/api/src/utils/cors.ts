...
// Origins are lowercased everywhere (hostnames are case-insensitive) so the
// exact-match comparison below works even when env/infra values are mixed-case.
...
const getTenantOrigins = async (dbBase: DatabasePg): Promise<string[]> => {
  ...
  const tenantHosts = queriedTenants
    .map((tenant) => tenant.host?.toLowerCase())
