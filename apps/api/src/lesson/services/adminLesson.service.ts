...
export class AdminLessonService {
  ...
  async createLessonForChapter(data: CreateLessonBody, currentUser: CurrentUser) {
    ...
    await this.validateAccess("chapter", currentUser.permissions, currentUser.userId, data.chapterId);
  ...
  async updateAiMentorLesson(
    ...
    await this.validateAccess("lesson", currentUser.permissions, currentUser.userId, id);
  ...
  async removeLesson(lessonId: UUIDType, currentUser: CurrentUser) {
    ...
    await this.validateAccess("lesson", currentUser.permissions, currentUser.userId, lessonId);
  ...
  async updateLessonDisplayOrder(lessonObject: {
    ...
    await this.validateAccess(
      "lesson",
      lessonObject.currentUser.permissions,
  ...
  async uploadFileToLesson(
    ...
    if (lessonId) {
      await this.masterCourseService.assertCourseContentEditableByLessonId(lessonId);
      await this.validateAccess("lesson", currentUserPermissions, currentUserId, lessonId);
    }
    const fileTitle = {
      [language]: title,
    };
  ...
  async validateAccess(
    ...
    const hasAccess = isTenantManager(currentUserPermissions) || course.authorId === currentUserId;
