...
const applyEmailLayout = (rawHtml: string, primaryColor: string): string => {
  ...
  const topCardHtml = buildCard(
    topCornerRadius,
    `${CARD_PADDING_TOP_PX}px ${CARD_PADDING_X_PX}px ${hasBottom ? 0 : CARD_PADDING_BOTTOM_PX}px ${CARD_PADDING_X_PX}px`,
    topBlocksHtml,
  );
  ...
  const topSectionHtml =
    `<div style="background-color:${primaryColor};padding-top:${HEADER_PADDING_TOP_PX}px${hasBottom ? "" : `;padding-bottom:${HEADER_PADDING_TOP_PX}px`}">` +
    `${topCardHtml}</div>`;
