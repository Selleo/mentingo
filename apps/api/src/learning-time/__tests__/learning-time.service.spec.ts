...
describe("LearningTimeService (unit)", () => {
  ...
  describe("getLearningTimeStatistics", () => {
    it("should return statistics for a course", async () => {
      ...
      expect(stats.data.users).toHaveLength(2);
      expect(stats.pagination.totalItems).toBe(2);

      const user1Stats = stats.data.users.find((user) => user.id === user1.id);
      const user2Stats = stats.data.users.find((user) => user.id === user2.id);

      expect(user1Stats?.totalSeconds).toBe(100);
      expect(user2Stats?.totalSeconds).toBe(200);
      expect(user1Stats?.studentAvatarUrl).toBeNull();
      expect(user2Stats?.studentAvatarUrl).toBeNull();
    });
    ...
    it("should return empty statistics for course with no data", async () => {
      ...
      expect(stats.data.users).toEqual([]);
      expect(stats.pagination.totalItems).toBe(0);
    });
  });
