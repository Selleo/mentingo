...
const LICENSES_JSON_PATH =
  process.env.LICENSES_JSON_PATH || '/tmp/licenses.json';
...
function loadConfig() {
  ...
  const allowedPackages = normalizeStringArray(
...
for (const entry of allowedPackagesList) {
  ...
  if (at <= 0 || at === entry.length - 1) {
    exitWithError(
      `Invalid allowedPackages entry "${entry}", expected "name@version"`,
    );
  }
